...
import random
import numpy as np
from model_modified import Encoder, Encoder_sparse, Encoder_map, Encoder_sc
...
class GraphST():
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            self.hiden_feat, self.emb, ret, ret_a = self.model(self.features, self.features_a, self.adj, self.graph_neigh)
        ...
        with torch.no_grad():
            self.model.eval()
            if self.deconvolution:
                self.emb_rec = self.model(self.features, self.features_a, self.adj, self.graph_neigh)[1]
                ...
                if self.datatype in ['Stereo', 'Slide']:
                    ...
                    self.emb_rec = self.model(self.features, self.features_a, self.adj, self.graph_neigh)[1].detach().cpu().numpy()
    ...
    def fine_tuning(self,
        ...
        y = torch.tensor(y, dtype=torch.int64).to(self.device) # tensor
        x_train = x[idx_train]
        if y_train_num == 'with_noise':
            y_train_num = y[idx_train]
        else :
            y_train_num = torch.tensor(y_train_num, dtype=torch.int64).to(self.device)
        
        adj = self.adj
        adj = adj[idx_notnull][:, idx_notnull]
        adj_train = adj[idx_train][:, idx_train]
        
        graph_neigh_add = self.graph_neigh
        graph_neigh_add = graph_neigh_add[idx_notnull][:, idx_notnull]
        graph_neigh_add_train = graph_neigh_add[idx_train][:, idx_train]
        
        
        
        linear_relu_stack = nn.Sequential(
            nn.Linear(self.dim_input, 512),
            nn.ReLU(),
            nn.Linear(512, 512),
            nn.ReLU(),
            # nn.Linear(256, 512),
        ...
        optimizer_1 = torch.optim.Adam(self.model.parameters(), self.learning_rate, weight_decay=self.weight_decay)
        optimizer_2 = torch.optim.SGD(linear_relu_stack.parameters(), lr=1e-3)
        
        print('Fine tuning... ...')
        self.model.train()
        linear_relu_stack.train()
        for epoch in tqdm(range(N_EPOCHS)):
            
            x_train_a = permutation(x_train)
            tmp_hiden_feat, tmp_emb, tmp_ret, tmp_ret_a = self.model(x_train, x_train_a, adj_train, graph_neigh_add_train)
            pred = linear_relu_stack(tmp_emb)
            
            loss_sl_1 = loss_CSL(tmp_ret, self.label_CSL[idx_notnull][idx_train])
            loss_sl_2 = loss_CSL(tmp_ret_a, self.label_CSL[idx_notnull][idx_train])
            loss_3 = loss_fn(pred, y_train_num)
            
            loss = W1*loss_3 + W2*(loss_sl_1 + loss_sl_2)
            
            early_stoping = torch.sum(pred.argmax(1) == y_train_num)/y_train_num.shape[0]
        ...
        print('Fine tuning has been done.')
        
        with torch.no_grad():
            print(epoch)
            self.model.eval()
            x_a = permutation(x)
            hiden_feat, emb = self.model(x, x_a, adj, graph_neigh_add)[0:2]
            pred = linear_relu_stack(emb)
            
            return pred, hiden_feat, emb, epoch
